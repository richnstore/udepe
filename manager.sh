...
sync_and_clean() {
    local today=$(date +%s)
    local changed=false
    if ! jq empty "$CONFIG_FILE" 2>/dev/null || ! jq empty "$META_FILE" 2>/dev/null; then return; fi
    ...
    if [ "$changed" = true ]; then /usr/bin/systemctl restart "$SERVICE_NAME" >/dev/null 2>&1; fi
}
...
draw_header() {
    ...
    local OS=$(lsb_release -ds 2>/dev/null || echo 'Linux')
    ...
    while true; do
        ...
        case $choice in
            ...
            5|05) 
                ...
                cp "$CONFIG_FILE" /tmp/config.json; curl -s -F chat_id="$TG_CHAT_ID" -F document=@/tmp/config.json "https://api.telegram.org/bot$TG_BOT_TOKEN/sendDocument" > /dev/null
                echo "Backup Sent!"; rm -f /tmp/config.json; sleep 1; fi ;;
            6|06) 
                clear; echo -e "${C}┏━━━━━━━━━━━━━${Y} RESTORE TELEGRAM ${C}━━━━━━━━━━━━┓${NC}"
                ...
                else echo -e "  ${R}File tidak ditemukan!${NC}"; fi; fi
                echo -e "${C}┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛${NC}"; sleep 2 ;;
            7|07) 
